	...
	<div class="Breakdowns">
		<div class="breakdown" v-for="breakdown in Breakdowns" :key="breakdown.id">
			<div class="breakdown-details">
				<h3 class="breakdownName">Attraction name: {{ breakdown.attractionName }}</h3>
				<h3>Broken down on: {{ breakdown.date }}</h3>
				<h3 :class="{ resolved: breakdown.resolved, unresolved: !breakdown.resolved }"><span>Attraction is:</span> {{ breakdown.resolved ? "operational" : "broken" }}</h3>
			</div>
